...
function c515242582.initial_effect(c)
	...
	e1:SetTarget(c515242582.tg)
	e1:SetOperation(c515242582.op)
	e1:SetCountLimit(1)
	c:RegisterEffect(e1)
	--Tiny Pony Search
	...
	e5:SetCondition(c515242582.dmcon)
	e5:SetTarget(c515242582.dmtg)
...
function c515242582.filter1(c,e,tp)
	...
	return c:IsFaceup() and c:IsSetCard(0x666) and c:IsAbleToDeckOrExtraAsCost()
		and Duel.IsExistingMatchingCard(c515242582.filter2,tp,LOCATION_DECK,0,1,nil,code,e,tp)
end
function c515242582.filter2(c,code,e,tp)
	return c:IsSetCard(0x666) and c:GetCode()~=code and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c515242582.tg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c515242582.dmcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsReason(REASON_DESTROY)
end
function c515242582.dmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c515242582.dmfilter,tp,LOCATION_EXTRA+LOCATION_GRAVE,0,1,nil) end
...
function c515242582.filter(c)
	return c:IsCode(515242564) and c:IsAbleToHand()
end
function c515242582.destg1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c515242582.filter,tp,0x51,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,0x51)
end
function c515242582.desop1(e,tp,eg,ep,ev,re,r,rp,chk)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local tg=Duel.SelectMatchingCard(tp,c515242582.filter,tp,0x51,0,1,1,nil):GetFirst()
	if tg then
